# traefik_k8s_docker_provider
use traefik use  k8s and docker provider create argocd in k8s 

# 紀錄Traefik 同時使用 K3d + Docker Provider
...
```bash
sudo k3d kubeconfig merge derek-cluster --kubeconfig-merge-default
```
...
```installation/argocd-cmd-params-cm-patch.yml
apiVersion: v1
kind: ConfigMap
metadata:
    ...
    - kind: Rule
      ...
      services:
        - name: argocd-server
          port: 80
    - kind: Rule
      match: Host(`argo.localhost`) && Headers(`Content-Type`, `application/grpc`)
      priority: 11
      services:
        - name: argocd-server
          port: 80
...
```
>  kubectl apply -f argo_ingress_route.yaml
```
